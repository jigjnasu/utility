...
#ifndef UTILITY_ALGORITHMS_SORT_MERGESORT_H_
#define UTILITY_ALGORITHMS_SORT_MERGESORT_H_
...
template <typename T>
void ua::MergeSort<T>::sort(std::string& data) const {
    m_sort(data, 0, data.size() - 1);
}
...
template <typename T>
void ua::MergeSort<T>::m_sort(std::string& data, std::size_t start, std::size_t end) const {
    if (start < end) {
        const std::size_t mid = (start + end) / 2;
        m_sort(data, start, mid);
        m_sort(data, mid + 1, end);
...
template <typename T>
void ua::MergeSort<T>::m_merge(std::string& data, std::size_t start, std::size_t mid, std::size_t end) const {
    const std::string left = data.substr(start, mid - start + 1);
    const std::string right = data.substr(mid + 1, end - mid);
...
#endif // UTILITY_ALGORITHMS_SORT_MERGESORT_H_
